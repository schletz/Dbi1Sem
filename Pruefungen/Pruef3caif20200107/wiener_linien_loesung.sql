...
DROP TABLE IF EXISTS Steig;
DROP TABLE IF EXISTS Haltestelle;
DROP TABLE IF EXISTS Linie;

CREATE TABLE Linie (
    Linien_id       INT         PRIMARY KEY,
    Bezeichnung     VARCHAR(16) NOT NULL UNIQUE,
    Reihenfolge     INT         NOT NULL,
    Echtzeit        INT         NOT NULL DEFAULT 0,
    Verkehrsmittel  VARCHAR(32) NOT NULL
    CHECK (Verkehrsmittel IN ('ptBusCity', 'ptBusNight', 'ptBusRegion', 'ptMetro', 'ptTrainS', 'ptTram', 'ptTramVRT', 'ptTramWLB')),
    CHECK (Echtzeit IN (0, 1))
);


CREATE TABLE Haltestelle (
    Haltestellen_id INT         PRIMARY KEY,
...
CREATE TABLE Steig (
    Steig_id            INT         PRIMARY KEY,
    Fk_linien_id        INT         NOT NULL REFERENCES Linie(Linien_id),
    Fk_haltestellen_id  INT         NOT NULL REFERENCES Haltestelle(Haltestellen_id),
    Richtung            CHAR(1)     NOT NULL,
    Reihenfolge         INT         NOT NULL,
    Steig               VARCHAR(16),
    CHECK (Richtung IN ('H', 'R'))
);
